...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(t) => t,
        Err(err) => match err {
            TicketNewError::TitleNotValid { msg } => panic!("{}", msg),
            TicketNewError::DescriptionNotValid {msg: _} => Ticket::new(title, "Description not provided".into(), status).unwrap()
        }
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleNotValid { msg: "Title cannot be empty".to_string() });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleNotValid { msg: "Title cannot be longer than 50 bytes".to_string() });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionNotValid { msg: "Description cannot be empty".to_string() });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionNotValid { msg: "Description cannot be longer than 500 bytes".to_string() });
        }
