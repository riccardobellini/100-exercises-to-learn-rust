    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let first_handle = tokio::spawn(send_fixed_reply(first, reply.clone()));
    let second_handle = tokio::spawn(send_fixed_reply(second, reply.clone()));
    let (_, _) = tokio::join!(first_handle, second_handle);
}

async fn send_fixed_reply<T>(listener: TcpListener, reply: Arc<T>) -> Result<(), anyhow::Error>
where
    T: Display + Send + Sync + 'static,
{
    loop {
        ...
        tokio::spawn(async move {
            ...
            writer.write_all(reply_clone.to_string().as_bytes()).await.unwrap();
        });
    }
}
